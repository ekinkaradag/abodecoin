...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Abodecoin " + FormatFullVersion();
...
unsigned int pnSeed[] =
{
    0x0100007f //IP address of localhost
    //Provide node IPs here
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Abodecoin is probably already running."), addrBind.ToString().c_str());
        else
