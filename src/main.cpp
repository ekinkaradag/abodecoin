...
#if defined(NDEBUG)
# error "Abodecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x0091f199b71f60bb93c4f1c78cebdf6f1ed3719ef12b09a80e0bf11da1aad282");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Abodecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Abodecoin Signed Message:\n";
...
{
    // Abodecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 630000); // Abodecoin: 630k blocks

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 24 * 60 * 60; // Abodecoin: 1 day
static const int64 nTargetSpacing = 5 * 60; // Abodecoin: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Abodecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Abodecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xe2b83742d777365e528715bfa8cc2860ebf94954496fe2f940ab6a4a39cf98cc");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Instagram account @abodecoin shares its first post 9/Jun/2018";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04297c8a1263852bd8bf9eaf0d42e0c8c8d4ac6aefbd75f8333536f23ea227fceb87ad40660ed19390418d33eec4e316a209ad0551f3b3cfb787585a6b31b21dd6") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1528551518;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2089506876;

        if (fTestNet)
        {
            block.nTime    = 1528551488;
            block.nNonce   = 385491394;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xccfea207e5da506fd3e7253db5ca6c7bde63b2003faa2d9cded6eee5ab83db3f"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc3, 0xb2, 0xda }; // Abodecoin: change the last digits randomly
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AbodecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AbodecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AbodecoinMiner(CWallet *pwallet)
{
    printf("AbodecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("abodecoin-miner");
    ...
    try { loop {
        ...
        printf("Running AbodecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AbodecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AbodecoinMiner, pwallet));
}
